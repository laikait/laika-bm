...
// Phase 49. Existing is not enough: `php laika nginx:make` regenerates
// nginx.conf from the framework's template, which knows nothing about
// modules/, and drops the deny without a word. It happened once already.
$checked++;
$nginx = (string) @file_get_contents($stage . '/nginx.conf');

if (!preg_match('#location\s+~\s*\^/modules\(/\|\$\)\s*\{\s*deny all;#', $nginx)) {
    fault('INSECURE nginx.conf has no `location ~ ^/modules(/|$) { deny all; }` - regenerated by nginx:make? Module credentials would be served to nginx users.');
}

// Phase 49. The licence travels with the code it licenses.
must_exist('LICENSE', 'The proprietary licence. An archive without it is code with no stated terms.');

// Phase 49. `php laika nginx:server` writes a server block for THIS machine -
// its document root, its PHP-FPM socket. Useless on an operator's server, and
// it names the build machine's paths.
must_not_exist('server.conf', 'Machine-specific nginx server block written by `php laika nginx:server`.');
...
// ---------------------------------------------------------------------------
// Phase 49. The session table. Sessions live in the database, and laika-session
// does not register its schema as a resource, so the installer creates the
// table by name and a migration creates it for older installs. Lose either and
// the first request after the wizard - or after an update - is a fatal.
// ---------------------------------------------------------------------------
must_exist(
    'vendor/laikait/laika-session/src/Schema/SessionSchema.php',
    'The sessions table\'s shape. The installer and M202609190100CreateSessionsTable both build it from here.'
);
must_exist(
    'vendor/laikait/laika-bm/src/Migration/M202609190100CreateSessionsTable.php',
    'Creates the sessions table on an install made before Phase 49. Without it that install cannot sign anybody in.'
);

$checked++;
$installer = (string) @file_get_contents($stage . '/vendor/laikait/laika-bm/src/Install/Installer.php');

if (!str_contains($installer, 'SessionSchema::class')) {
    fault('BROKEN   Installer::migrate() no longer creates the sessions table - a fresh install fails on its first request.');
}
